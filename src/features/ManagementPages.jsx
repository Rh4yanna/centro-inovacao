...
export function RepresentativeDetail({ representatives, institutions, onDeleted }) {
  ...
  return <><Heading title={person.name}>{person.role}</Heading><Card title="Dados pessoais"><dl className="info-grid">{[['CPF',person.cpf],['E-mail',person.email],['Telefone',person.phone],['Status',person.status],['Instituição',institution?.name || person.institutionName],['Endereço',[person.street,person.number,person.city,person.state].filter(Boolean).join(', ')]].map(([k,v]) => <div key={k}><dt>{k}</dt><dd>{v || 'Não informado'}</dd></div>)}</dl></Card><Links personId={id} institutions={institutions}/><Attendance personId={id}/><div className="form-actions"><DeleteRepresentative person={person} onDeleted={onDeleted}/><Link className="button" to="/representantes">Voltar</Link><Permissao><Link className="button primary" to={`/representantes/${id}/editar`}>Editar representante</Link></Permissao></div></>;
}
